...
wing = Wing(name="Main Wing", sections=wing_sections, symmetric=true,control_surfaces=[aileron])
...
horizontal_stabilizer = Wing(name="Horizontal Stabilizer", sections=hs_sections, symmetric=true,control_surfaces=[elevator])
...
vertical_stabilizer = Wing(name="Vertical Stabilizer", sections=vs_sections,symmetric=false,control_surfaces=[rudder])
...
fuse_sections = [FuselageSection(radius=radii[i],center=[xc[i], 0, zc[i]],shape=shapes[i]) for i in eachindex(xc)]
